...
const SuccesPage = () => {
    ...
    let cartData = JSON.parse(localStorage.getItem("checkoutData")) ? JSON.parse(localStorage.getItem("checkoutData"))
        : [];
    ...
    return (
        <section className="order-success-container">
            ...
            <img src={SuccessPageIllustration} alt="Order is placed!" className="checkout-success-image"/>

            <section className="customer-information">
                <span>{("Hallo" + " " + cartData.firstName)}</span>
                <p>We hebben jou bestelling goed ontvangen, en gaan voor je aan de slag!</p>
                <p>Zodra jou bestelling klaar is in ons magazijn, ontvang je van ons een mailtje om je bestelling te volgen!</p>
                <p>Mocht je toch nog vragen hebben, onze klantenservice staat altijd voor je klaar!</p>
            </section>
